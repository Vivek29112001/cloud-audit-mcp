...
class AWSResourceDiscoveryService:
    ...
    async def _discover_region(
        ...
        for item in raw_resources:
            ...
            # Resource Explorer may expose multiple descriptive fields.
            # For customer inventory, prefer the service namespace encoded in
            # the resource ARN itself. This prevents MCP wrapper/control values
            # (for example content types or API client identifiers) from being
            # surfaced as customer AWS services. No service allow/deny list is
            # used here.
            service = self._canonical_service(
                arn=arn,
                resource_type=item.get(
                    "ResourceType"
                ),
                reported_service=item.get(
                    "Service"
                ),
            )
    ...
    def _canonical_service(
        *,
        ...
        reported_service: str | None,
    ) -> str | None:
        """
        Derive a customer-resource service namespace from resource evidence.

        Priority:
        1. ARN service segment (strongest evidence).
        2. Resource type namespace when it is explicitly namespaced.
        3. Resource Explorer's Service field as a final fallback.

        This is intentionally generic. It does not contain a predefined AWS
        service catalog or a list of scanner services to hide.
        """
        if arn:
            parts = str(arn).split(":", maxsplit=5)
            if len(parts) >= 3 and parts[0].lower() == "arn":
                namespace = parts[2].strip().lower()
                if namespace:
                    return namespace

        value = str(resource_type or "").strip()
        if value and "::" not in value and ":" in value:
            namespace = value.split(":", maxsplit=1)[0].strip().lower()
            if namespace:
                return namespace

        value = str(reported_service or "").strip().lower()
